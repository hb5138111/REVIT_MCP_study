...
[CmdletBinding()]
param([Parameter(Mandatory)][string]$ReversibleDirectory,[Parameter(Mandatory)][string]$EarthworkReversibleDirectory)
$ErrorActionPreference='Stop'
$repo=Split-Path $PSScriptRoot -Parent
Set-Location $repo
$run=(Resolve-Path -LiteralPath $ReversibleDirectory).Path
$release=Join-Path $repo 'test-artifacts/v0521'
$state=Get-Content docs/productization/v0521-report.json -Raw|ConvertFrom-Json
...
$receipt=[ordered]@{Kind='FORMAL_V0521_RELEASE';Status='PENDING';BuildSHA256=$buildHash;Timestamp=[DateTimeOffset]::UtcNow;Backup=[IO.Path]::GetRelativePath($repo,$run);Rollback='NOT_NEEDED'}
try {
