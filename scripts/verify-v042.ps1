# v0.4.2 source, runtime and release gates. Runtime staging is reversible; final deployment is explicit.
[CmdletBinding()]
param([switch]$Runtime,[switch]$Deploy,[string]$RuntimeEvidence,[string]$ProjectTemplate)
$ErrorActionPreference='Stop'
$repo=Split-Path $PSScriptRoot -Parent
Set-Location $repo
$out=Join-Path $repo 'test-artifacts/v042'
New-Item -ItemType Directory -Path $out -Force|Out-Null
function Gate($name,[scriptblock]$action){ & $action *> (Join-Path $out "$name.log");if($LASTEXITCODE -ne 0){throw "$name failed; inspect test-artifacts/v042/$name.log"} }
...
if($Runtime){
    ...
    $lines=& pwsh -NoProfile -File scripts/test-reversible-gate-c.ps1 @templateArguments
    $lines|Write-Output
...
node scripts/write-v042-report.cjs $RuntimeEvidence
if($LASTEXITCODE -ne 0){throw 'Report generation failed'}
$report=Get-Content docs/productization/v042-report.json -Raw|ConvertFrom-Json
if(@($report.Gates.PSObject.Properties|Where-Object{$_.Value.Status -ne 'PASS'}).Count){throw 'A required release gate is not PASS'}
if($Deploy){
    & pwsh -NoProfile -File scripts/publish-v042.ps1 -ReversibleDirectory $RuntimeEvidence
    if($LASTEXITCODE -ne 0){throw 'Formal deployment failed; inspect deployment receipt and rollback'}
    node scripts/write-v042-report.cjs $RuntimeEvidence test-artifacts/v042/deployment.json
