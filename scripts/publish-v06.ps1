...
[CmdletBinding()]
param([Parameter(Mandatory)][string]$ReversibleDirectory,[Parameter(Mandatory)][string]$EarthworkReversibleDirectory,[Parameter(Mandatory)][string]$DrawingReversibleDirectory)
$ErrorActionPreference='Stop'
$repo=Split-Path $PSScriptRoot -Parent
Set-Location $repo
$run=(Resolve-Path -LiteralPath $ReversibleDirectory).Path
$release=Join-Path $repo 'test-artifacts/v06'
$state=Get-Content docs/productization/v06-report.json -Raw|ConvertFrom-Json
...
$receipt=[ordered]@{Kind='FORMAL_V06_RELEASE';Status='PENDING';BuildSHA256=$buildHash;Timestamp=[DateTimeOffset]::UtcNow;Backup=[IO.Path]::GetRelativePath($repo,$run);Rollback='NOT_NEEDED'}
try {
