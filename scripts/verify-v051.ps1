...
node scripts/write-v051-report.cjs $RuntimeEvidence
if($LASTEXITCODE){throw 'Report failed'}
if($Deploy){
    if(-not $RuntimeEvidence){throw 'No runtime evidence'}
    pwsh -NoProfile -File scripts/publish-v051.ps1 -ReversibleDirectory $RuntimeEvidence
    if($LASTEXITCODE){throw 'Formal deployment failed'}
    node scripts/write-v051-report.cjs $RuntimeEvidence test-artifacts/v051/deployment.json
