...
$release=Join-Path $repo 'test-artifacts/v052'
$state=Get-Content docs/productization/v052-report.json -Raw|ConvertFrom-Json
...
if($nativeSnapshot.Rollback -ne 'PASS' -or $nativeSnapshot.BuildSHA256 -ne $buildHash -or $native.BuildSHA256 -ne $buildHash -or $native.Status -ne 'PASS' -or $native.Failed -ne 0 -or $native.Passed -lt 38){throw 'Earthwork Native runtime proof does not match release build'}
...
$receipt=[ordered]@{Kind='FORMAL_V052_RELEASE';Status='PENDING';BuildSHA256=$buildHash;Timestamp=[DateTimeOffset]::UtcNow;Backup=[IO.Path]::GetRelativePath($repo,$run);Rollback='NOT_NEEDED'}
try {
