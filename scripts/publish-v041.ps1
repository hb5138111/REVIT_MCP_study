...
$release=Join-Path $repo 'test-artifacts/v041'
$state=Get-Content docs/productization/v041-report.json -Raw|ConvertFrom-Json
...
$receipt=[ordered]@{Kind='FORMAL_V041_RELEASE';Status='PENDING';BuildSHA256=$buildHash;Timestamp=[DateTimeOffset]::UtcNow;Backup=[IO.Path]::GetRelativePath($repo,$run);Rollback='NOT_NEEDED'}
try {
