...
[CmdletBinding()]
param([Parameter(Mandatory)][string]$ReversibleDirectory,[Parameter(Mandatory)][string]$EarthworkReversibleDirectory,[Parameter(Mandatory)][string]$DrawingReversibleDirectory,[Parameter(Mandatory)][string]$JourneyReversibleDirectory)
$ErrorActionPreference='Stop'
$repo=Split-Path $PSScriptRoot -Parent
Set-Location $repo
$run=(Resolve-Path -LiteralPath $ReversibleDirectory).Path
$release=Join-Path $repo 'test-artifacts/v061'
$state=Get-Content docs/productization/v061-report.json -Raw|ConvertFrom-Json
...
$receipt=[ordered]@{Kind='FORMAL_V061_RELEASE';Status='PENDING';BuildSHA256=$buildHash;Timestamp=[DateTimeOffset]::UtcNow;Backup=[IO.Path]::GetRelativePath($repo,$run);Rollback='NOT_NEEDED'}
try {
