...
[CmdletBinding()]
param([Parameter(Mandatory)][string]$ReversibleDirectory,[Parameter(Mandatory)][string]$EarthworkReversibleDirectory,[Parameter(Mandatory)][string]$DrawingReversibleDirectory,[Parameter(Mandatory)][string]$JourneyReversibleDirectory,[Parameter(Mandatory)][string]$ActualCadReversibleDirectory)
$ErrorActionPreference='Stop'
$repo=Split-Path $PSScriptRoot -Parent
Set-Location $repo
$run=(Resolve-Path -LiteralPath $ReversibleDirectory).Path
$release=Join-Path $repo 'test-artifacts/v0611'
$state=Get-Content docs/productization/v0611-report.json -Raw|ConvertFrom-Json
$snapshot=Get-Content (Join-Path $run 'reversible.json') -Raw|ConvertFrom-Json
$nativeSnapshot=Get-Content (Join-Path $EarthworkReversibleDirectory 'reversible.json') -Raw|ConvertFrom-Json
$native=Get-Content (Join-Path $nativeSnapshot.RuntimeDirectory 'earthwork-workflow.json') -Raw|ConvertFrom-Json
$drawingSnapshot=Get-Content (Join-Path $DrawingReversibleDirectory 'reversible.json') -Raw|ConvertFrom-Json
$drawing=Get-Content (Join-Path $drawingSnapshot.RuntimeDirectory 'drawing-runtime.json') -Raw|ConvertFrom-Json
$journeySnapshot=Get-Content (Join-Path $JourneyReversibleDirectory 'reversible.json') -Raw|ConvertFrom-Json
$journey=Get-Content (Join-Path $journeySnapshot.RuntimeDirectory 'drawing-c4-runtime.json') -Raw|ConvertFrom-Json
...
if($nativeSnapshot.Rollback -ne 'PASS' -or $nativeSnapshot.BuildSHA256 -ne $buildHash -or $native.BuildSHA256 -ne $buildHash -or $native.Status -ne 'PASS' -or $native.Failed -ne 0 -or $native.Passed -lt 61 -or $native.FixtureVersion -ne 'earthwork-native-2'){throw 'Earthwork Native runtime proof does not match release build'}
if($drawingSnapshot.Rollback -ne 'PASS' -or $drawingSnapshot.BuildSHA256 -ne $buildHash -or $drawing.BuildSHA256 -ne $buildHash -or $drawing.Status -ne 'PASS' -or $drawing.Failed -ne 0 -or $drawing.Passed -lt 46){throw 'Drawing Native runtime proof does not match release build'}
if($journeySnapshot.Rollback -ne 'PASS' -or $journeySnapshot.BuildSHA256 -ne $buildHash -or $journey.BuildSHA256 -ne $buildHash -or $journey.Status -ne 'PASS' -or $journey.GateC4 -ne 'PASS' -or $journey.Failed -ne 0){throw 'C4 production user journey is missing, failed or stale'}
foreach($case in @('A','B','C','D','E','F','G','H')){if($journey.Cases.$case -ne $true){throw "C4 case $case is not PASS"}}
$actualSnapshot=Get-Content (Join-Path $ActualCadReversibleDirectory 'reversible.json') -Raw|ConvertFrom-Json
$actualJourney=Get-Content (Join-Path $actualSnapshot.RuntimeDirectory 'drawing-c4-runtime.json') -Raw|ConvertFrom-Json
if($actualJourney.Status -ne 'PASS' -or $actualJourney.BuildSHA256 -ne $buildHash -or $actualJourney.Failed -ne 0 -or -not @($actualJourney.Assertions|Where-Object{$_.TestName -eq 'actual_source_unchanged' -and $_.Passed}).Count){throw 'Actual source integrity proof is absent or failed'}
$actual=Get-Content (Join-Path $actualSnapshot.RuntimeDirectory 'actual-cad-uat.json') -Raw|ConvertFrom-Json
if($actualSnapshot.Rollback -ne 'PASS' -or $actualSnapshot.BuildSHA256 -ne $buildHash -or $actual.BuildSHA256 -ne $buildHash -or $actual.Status -ne 'ACTUAL_DWG_UAT_PASS'){throw 'Actual DWG UAT is missing, failed or stale'}
foreach($proof in @($journey,$actual)){foreach($case in @('I_Candidates','I_ProductionJourney','I_Idempotency','I_ManualOverride','J_IndependentProfiles','J_FamilyReadback')){if($proof.Cases.$case -ne $true){throw "CAD journey case $case is not PASS"}}}
if($actual.Construction.ProfileGuid -eq $actual.AsBuilt.ProfileGuid -or $actual.Construction.Blueprint.TitleBlockTypeId -eq $actual.AsBuilt.Blueprint.TitleBlockTypeId){throw 'Actual CAD profiles are not independent'}
if(@($actual.Construction.Blueprint.CadGeometryIds|Where-Object{$_ -in $actual.AsBuilt.Blueprint.CadGeometryIds}).Count){throw 'Actual CAD profiles contain overlapping source geometry'}
$visual=Get-Content test-artifacts/v0611/actual-visual-review.json -Raw|ConvertFrom-Json
if($visual.Status -ne 'PASS' -or $visual.BuildSHA256 -ne $buildHash){throw 'Actual visual review is missing or stale'}
$c5=Get-Content test-artifacts/v0611/cad-c5.json -Raw|ConvertFrom-Json
if($c5.GateC5 -ne 'PASS' -or $c5.Failed -ne 0 -or $c5.Passed -lt 39 -or $c5.BuildSHA256 -ne $buildHash){throw 'C5 is missing, failed or stale'}
foreach($file in $c5.SourceFiles){if((Get-FileHash -LiteralPath $file.Path).Hash -ne $file.SHA256){throw "C5 source changed: $($file.Path)"}}
...
$receipt=[ordered]@{Kind='FORMAL_V0611_RELEASE';Status='PENDING';BuildSHA256=$buildHash;Timestamp=[DateTimeOffset]::UtcNow;Backup=[IO.Path]::GetRelativePath($repo,$run);Rollback='NOT_NEEDED'}
try {
    ...
    if((Get-FileHash (Join-Path $build 'ThirdPartyNotices.txt')).Hash -ne (Get-FileHash (Join-Path $target 'ThirdPartyNotices.txt')).Hash){throw 'Third-party notices differ'}
    $receipt.Status='PASS'
}
catch {
