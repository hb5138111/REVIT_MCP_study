...
$release=Join-Path $repo 'test-artifacts/v05'
$state=Get-Content docs/productization/v05-report.json -Raw|ConvertFrom-Json
...
$receipt=[ordered]@{Kind='FORMAL_V05_RELEASE';Status='PENDING';BuildSHA256=$buildHash;Timestamp=[DateTimeOffset]::UtcNow;Backup=[IO.Path]::GetRelativePath($repo,$run);Rollback='NOT_NEEDED'}
try {
